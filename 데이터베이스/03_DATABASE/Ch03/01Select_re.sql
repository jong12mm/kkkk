...
select userid,prodname from buytbl where price >=50 and price<=50 ;
...
-- 성시경보다 나이가 많은(birthyear)가 많은 모든 열의값 출력
select * from usertbl 
where birthyear > (select birthyear from usertbl where name='성시경');
