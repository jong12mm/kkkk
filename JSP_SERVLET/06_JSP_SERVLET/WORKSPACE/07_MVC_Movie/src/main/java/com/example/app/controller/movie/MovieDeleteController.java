...
public class MovieDeleteController  implements SubController{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String deleteIdStr = request.getParameter("movieId");
			...
			if (deleteIdStr != null && !deleteIdStr.isEmpty()) {
	            ...
	            // 영화 삭제 작업 수행
	            movieService.removeMovie(deleteId);
	        }
			//1
			...
			request.getRequestDispatcher("/WEB-INF/view/movie/delete.jsp").forward(request, response);
